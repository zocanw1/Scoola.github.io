...
<div class="editorial-header">
    ...
    <h1 class="display-title">Tambah Presensi</h1>
    <p class="text-body" style="color: var(--color-graphite); max-width: 600px;">
        Catat kehadiran siswa secara manual untuk sesi mata pelajaran tertentu.
    </p>
</div>
...
<div style="max-width: 720px; margin-bottom: 64px;">
    <form action="{{ route('guru.presensi.buka') }}" method="POST">
        ...
        <div style="margin-bottom: 48px;">
            ...
            <input type="date" name="tanggal" style="width: 100%; border: none; border-bottom: 1px solid var(--color-hairline); padding: 12px 0; font-family: var(--font-family-base); font-size: 15px; outline: none; background: transparent; color: var(--color-ink);" required value="{{ date('Y-m-d') }}">
        </div>
        ...
        <div style="margin-bottom: 48px;">
            ...
            <div style="position: relative; border-bottom: 1px solid var(--color-hairline);">
                <select name="kd_jp" style="width: 100%; border: none; padding: 12px 0; font-family: var(--font-family-base); font-size: 15px; outline: none; background: transparent; cursor: pointer; appearance: none; color: var(--color-ink);" required>
                    ...
                    @foreach($jadwal as $j)
                        <option value="{{ $j->kd_jp }}">{{ $j->kd_jp }}</option>
                    @endforeach
                </select>
        ...
        <div style="margin-bottom: 48px;">
            ...
            <input type="time" name="jam_masuk" style="width: 100%; border: none; border-bottom: 1px solid var(--color-hairline); padding: 12px 0; font-family: var(--font-family-base); font-size: 15px; outline: none; background: transparent; color: var(--color-ink);" required value="{{ date('H:i') }}">
        </div>
