        ...
        <div class="editorial-header" style="margin: 0; display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 24px;">
            <div>
                ...
                <p class="text-body" style="color: var(--color-graphite); font-size: 15px; line-height: 1.6; margin: 0; max-width: 600px;">
                    ...
                    <br><strong>Minggu: {{ $startOfWeek->format('d M Y') }} - {{ $endOfWeek->format('d M Y') }}</strong>
                    @endif
                </p>
            </div>
            
            <form action="{{ route('admin.rekap.index') }}" method="GET" style="display: flex; gap: 12px; align-items: flex-end;">
                ...
                <div style="display: flex; flex-direction: column; gap: 6px;">
                    ...
                    <input type="date" name="tanggal" value="{{ $tanggalInput ?? now()->toDateString() }}" class="form-control" style="height: 44px; border-radius: 8px; border: 1px solid var(--color-hairline); padding: 0 12px;">
                </div>
                ...
                @if($selectedKelas)
                <button type="submit" formaction="{{ route('admin.rekap.export') }}" formmethod="GET" style="height: 44px; padding: 0 24px; border-radius: 8px; font-size: 14px; font-weight: 600; border: 1px solid #107c41; background: #107c41; color: white; cursor: pointer; display: flex; align-items: center; gap: 8px;">
            ...
            <table style="width: 100%; min-width: 1200px; border-collapse: collapse; font-size: 12px; font-family: sans-serif; text-align: center;">
                <thead>
                    ...
                    <tr>
                        @foreach($hariList as $hari)
                            @for($i = 1; $i <= 12; $i++)
                                <th style="border: 1px solid #000; padding: 4px; background: #fdfdfd;">{{ $i }}</th>
                            @endfor
                        @endforeach
                    </tr>
                    <tr>
                            ...
                            @for($i = 1; $i <= 12; $i++)
                                ...
                                <th style="border: 1px solid #000; padding: 4px; font-size: 10px; background: #fff; font-weight: normal; vertical-align: top; width: 45px;">
                                    <div style="writing-mode: vertical-rl; text-orientation: mixed; height: 120px; margin: 0 auto; line-height: 1.2;">
                                        ...
                                        <span style="color: #666; font-size: 9px;">{{ $guruTampil }}</span>
                                    </div>
                                </th>
                            @endfor
                        @endforeach
                ...
                <tbody>
                    @forelse($siswas as $index => $siswa)
                        <tr>
                            ...
                            <td style="border: 1px solid #000; padding: 6px;">{{ $siswa->NIS }}</td>
                            <td style="border: 1px solid #000; padding: 6px; text-align: left;">{{ $siswa->nama_siswa }}</td>
                            <td style="border: 1px solid #000; padding: 6px;">{{ $siswa->jenis_kelamin ?? 'L' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ 4 + (count($hariList) * 12) }}" style="border: 1px solid #000; padding: 16px; text-align: center;">Belum ada data siswa di kelas ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
