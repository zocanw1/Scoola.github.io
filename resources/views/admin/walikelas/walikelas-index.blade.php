    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(340px, 1fr)); gap: var(--spacing-md);">
        ...
        <div class="card" style="background: #ffffff; padding: 40px; border-radius: 12px; border: 1px solid var(--color-hairline); position: relative; overflow: hidden;">
            ...
            <div style="padding-left: 12px;">
                ...
                <div style="font-size: 56px; color: var(--color-ink); font-weight: 400; letter-spacing: var(--tracking-tighter); line-height: 1;">{{ $totalKelas }}</div>
            </div>
        </div>
        ...
        <div class="card" style="background: #ffffff; padding: 40px; border-radius: 12px; border: 1px solid var(--color-hairline); position: relative; overflow: hidden;">
            ...
            <div style="padding-left: 12px;">
                ...
                <div style="font-size: 56px; color: var(--color-ink); font-weight: 400; letter-spacing: var(--tracking-tighter); line-height: 1;">{{ $terisi }}</div>
            </div>
        </div>
    </div>
        ...
        <table class="data-table responsive-table" style="margin: 0; width: 100%;" id="walikelasTable">
            ...
            <tbody>
                ...
                <tr class="walikelas-row">
                    <td data-label="Kelas" style="padding-left: 40px; font-weight: 600;">{{ $kls->nama_kelas }}</td>
                    ...
                    <td data-label="Aksi" style="text-align: right; padding-right: 40px;">
                        @if($kls->waliKelas)
                            <a href="{{ route('admin.walikelas.edit', $kls->id) }}" class="btn-ghost" style="height: 32px; font-size: 11px; padding: 0 16px; text-decoration: none; display: inline-flex; align-items: center;">Edit</a>
                        @else
                            <a href="{{ route('admin.walikelas.create') }}?kelas_id={{ $kls->id }}" class="btn-primary" style="height: 32px; font-size: 11px; padding: 0 16px; text-decoration: none; display: inline-flex; align-items: center;">Assign</a>
                        @endif
                    </td>
                </tr>
                @empty
...
<script>
    ...
    <div class="fab-container">
        <a href="{{ route('admin.walikelas.create') }}" class="btn-fab" title="Tambah Wali Kelas">
