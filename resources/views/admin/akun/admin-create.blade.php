...
<div class="editorial-header">
    ...
    <div class="card" style="background: #ffffff; padding: 32px; border-radius: 16px; border: 1px solid var(--color-hairline);">
        <div class="editorial-header" style="margin: 0;">
            ...
            <h1 class="display-title" style="font-size: 48px; font-weight: 400; letter-spacing: var(--tracking-tighter); margin: 8px 0 24px 0; text-transform: uppercase;">Tambah Administrator</h1>
...
<div class="card" style="background: #ffffff; padding: 48px; border-radius: 16px; border: 1px solid var(--color-hairline); margin-bottom: var(--spacing-section);">
    <form method="POST" action="{{ route('admin.akun.store') }}">
        ...
        <div style="margin-bottom: 48px;">
            <label class="text-micro-caps" style="display: block; margin-bottom: 8px; color: var(--color-stone);">Nama Lengkap</label>
            <input type="text" name="name" class="form-field" placeholder="System Administrator" required value="{{ old('name') }}" autofocus>
        </div>

        <div style="margin-bottom: 48px;">
            <label class="text-micro-caps" style="display: block; margin-bottom: 8px; color: var(--color-stone);">Email</label>
            <input type="email" name="email" class="form-field" placeholder="admin@scoola.id" required value="{{ old('email') }}">
        </div>
        ...
        <div class="form-actions">
            <button type="submit" class="btn-primary" style="height: 56px; padding: 0 48px; font-size: 15px;">Daftarkan Admin</button>
