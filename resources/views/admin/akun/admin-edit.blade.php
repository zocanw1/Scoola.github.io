...
<div class="mp-page">
    <div class="mp-hero-wrap">
        ...
        <section class="mp-hero">
            ...
            <h1 class="mp-title">Edit Admin</h1>
            <p class="mp-description">
                Perbarui profil hak akses untuk administrator {{ $admin->name }}.
            </p>
            </div>
        </section>
    </div>
    ...
    <section class="mp-form-card">
        <form method="POST" action="{{ route('admin.akun.update', $admin->id) }}">
            ...
            <div class="mp-form-grid">
                <div class="mp-field">
                    <label class="mp-label">Nama Lengkap Administrator</label>
                    <input type="text" name="name" class="mp-input" placeholder="Nama Admin" required value="{{ old('name', $admin->name) }}">
                </div>

                <div class="mp-field">
                    <label class="mp-label">Alamat Email</label>
                    <input type="email" name="email" class="mp-input" placeholder="Email" required value="{{ old('email', $admin->email) }}">
                </div>
